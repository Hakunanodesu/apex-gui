...
public sealed partial class MainWindow : GameWindow
{
    ...
    private static readonly string[] HomeSnapModeOptions = { "¿ª»ðÎü¸½", "Ãé×¼ + ¿ª»ðÎü¸½" };
    ...
    private void DrawHomeModelCombo(string id, float width = -1f)
    {
        ...
        if (_onnxModels.Count == 0)
        {
            ...
            ImGui.Combo(id, ref _onnxTopSelectedModelIndex, "é—‚ä½¸æ«éŸç‰ˆæ…¨çº¾å¬¨äº¹é–ºå±»å„²é‹ã„©æŸ•æ¿žãˆ¢ç¹ç»¶æ°¬ç•·ç‘™å‹«åž”?");
            ImGui.EndDisabled();
            return;
        }
