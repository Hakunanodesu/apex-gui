...
pub fn show_param_curve(
    ...
    ui.horizontal(|ui| {
        ...
        // x轴刻度：0、innersize、midsize
        let mid_size_val = mid_size.trim().parse::<f32>().unwrap_or(1.0); // 防止除0
        let inner_size_val = inner_size.trim().parse::<f32>().unwrap_or(0.0);
        let inner_x = if mid_size_val > 0.0 {
            (inner_size_val / mid_size_val).clamp(0.0, 1.0)
        } else {
            0.0
        };
        let mid_x = 1.0;
        ...
        let mid_y = mid_str.trim().parse::<f32>().unwrap_or(0.0).clamp(0.0, 1.0);
        ...
        let p2 = to_screen(mid_x, mid_y);
        let p3 = to_screen(mid_x, outer_y);
        ...
        painter.circle_filled(p3, 5.0, egui::Color32::LIGHT_GREEN);
        // 连线
